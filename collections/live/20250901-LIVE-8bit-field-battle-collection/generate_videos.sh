...
# ⚔️ Field Battle Collection - 完全動画生成スクリプト
...
BASE_DIR="$SCRIPT_DIR"
INPUT_IMAGE="$BASE_DIR/10-assets/thambnail-base.png"
MASTER_AUDIO="$BASE_DIR/01-master/00-filed-battle-master.wav"
INDIVIDUAL_DIR="$BASE_DIR/02-Individual-music"
OUTPUT_DIR="$BASE_DIR/03-Individual-movie"

echo "⚔️ Field Battle Collection 動画生成開始"
echo "📁 基本パス確認:"
echo "   ベース: $BASE_DIR"
echo "   メイン画像: $INPUT_IMAGE"
echo "   マスター音声: $MASTER_AUDIO"
echo "   個別音声: $INDIVIDUAL_DIR"
echo "   出力先: $OUTPUT_DIR"
echo ""
...
# 必要ファイル確認
echo "📁 ファイル存在確認中..."
check_file "$INPUT_IMAGE" "thumbnail-base.png" || exit 1
check_file "$MASTER_AUDIO" "マスター音声" || exit 1

# 個別音声ファイル数確認
TOTAL=$(find "$INDIVIDUAL_DIR" -name "*.wav" | wc -l | tr -d ' ')
if [ "$TOTAL" -eq 0 ]; then
    echo "❌ エラー: 個別音声ファイルが見つかりません"
    echo "   パス: $INDIVIDUAL_DIR"
    exit 1
fi
echo "🎵 個別音声ファイル: ${TOTAL}個確認"
echo ""

# マスター動画生成
MASTER_OUTPUT="$OUTPUT_DIR/00-master.mp4"
echo "🎬 マスター動画生成開始: $(basename "$MASTER_OUTPUT")"
...
ffmpeg -y -loop 1 -i "$INPUT_IMAGE" -i "$MASTER_AUDIO" \
...
for file in "$INDIVIDUAL_DIR"/*.wav; do
    ...
    # FFmpegをバックグラウンドで実行（プログレス情報付き・静止画版）
    ffmpeg -y -loop 1 -i "$INPUT_IMAGE" -i "$file" \
           -c:v libx264 -c:a aac -pix_fmt yuv420p -r 30 \
           -shortest -progress "pipe:1" \
...
echo "🎉 ⚔️ Field Battle Collection 動画生成完了！"
echo "📊 結果サマリー:"
echo "   📹 マスター動画: 1本生成完了"
...
echo ""
echo "🎮 次のステップ:"
echo "1. 生成された動画ファイルを確認"
echo "2. YouTube アップロード準備"
echo "3. サムネイル・概要欄の作成"
echo ""